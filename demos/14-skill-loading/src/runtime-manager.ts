...
export class RuntimeManager {
  ...
  constructor(
    ...
    private readonly skillRoot = join(projectRoot, "workspace/skills"),
...
export async function materializePatch(
  ...
  const template = await readFile(
    join(projectRoot, "patches/skill-loading.patch.yml"),
    "utf8",
  );
  const patch = template.replace("__SKILL_ROOT__", JSON.stringify(skillRoot));
  if (patch.includes("__SKILL_ROOT__")) {
    throw new Error("unresolved skill root in patch");
  }
  await mkdir(dshHome, { recursive: true });
  const output = join(dshHome, "skill-loading.resolved.patch.yml");
